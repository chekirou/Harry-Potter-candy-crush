...
void jouer(SDL_Surface *ecran, int option)
{
    ...
    Mix_Music *musique;
    musique = Mix_LoadMUS("suppsound.mp3");
    ...
    while(continuer)
    {
       ...
       if(departingTime - endTime <0)
        ...
        if(endTime- startTime >  500)
        {
            ...
            timetoChar(departingTime - endTime, &m, &s);
...
void arret(SDL_Surface *ecran, int score, int option, int mat[10][11], int time)
{
    ...
    message= TTF_RenderText_Blended(police, "fin du temps imparti", noir);
    felicitation= TTF_RenderText_Blended(police, "vous avez obtenu le meilleur score !!!", noir);
...
void humainMachine()
{
    ...
    SDL_Surface *ecran2 = NULL , *background = NULL, *shade = NULL, *shade2 = NULL, *time = NULL, *barre = NULL, *timeleft = NULL, *boutonhelp = NULL;
    ...
    int a,b, c, d, x,y,w,z, m,s;
    char TIME[4], Cscore[20];
    SDL_Color noir =  {0,0,0};
    ...
    recurevite(mat1, &RES1, NULL);
    recurevite(mat2, &RES2, NULL);
    ...
    while(continuer)
    {
        ...
        case SDL_MOUSEBUTTONDOWN:
            ...
            else if(event.button.x > Xarret2 && event.button.x < Xarret2 +50 && event.button.y > Yarret2 && event.button.y < Yarret2+50)
            {
                continuer = 0;
       ...
       if(departingTime - endTime <0)
        ...
        if(endTime- startTime >  500)
        {
            ...
            SDL_FreeSurface(timeleft);
            timeleft =TTF_RenderText_Blended(police,TIME, noir);
            sprintf(Cscore, "votre score :%d", RES1);

            //SDL_FreeSurface(Score);
            Score = TTF_RenderText_Blended(police, Cscore, noir);
            sprintf(Cscore, "score machine :%d", RES2);
            ...
            SDL_BlitSurface(background, NULL, ecran2, &pos);
                ...
                pos.x = Xcanvas3 - 10;
                pos.y = 0;
                SDL_BlitSurface(barre, NULL, ecran2, &pos);
                for(i = 0; i< 10; i++)
                {
                    for(j= 0; j< 11; j++)
                    {
