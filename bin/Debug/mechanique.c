...
void timetoChar(int time, char timechar[])
{
    ...
    time /= 1000;
    TimeTrans = time / 60;
    timechar[i]  = (TimeTrans % 10)+48;
    i++;
    timechar[i] = ':';
    TimeTrans = time % 60;
    timechar[i+1] = (TimeTrans /10)+48;
    timechar[i+2] = (TimeTrans %10)+48;
}
...
void recurevite(int mat[10][11], int *score)
{
    ...
    for(i = 0; i< 10; i++)
    {
        recurvite_ligne(i, mat, score);
        recurevite_colonne(i, mat, score);
    }
    recurevite_colonne(10, mat, score);
...
void recurvite_ligne(int ligne, int mat[10][11], int *score)
{
    int j,p, type, action2 =0, action3 = 0,Y;
        for(j =0; j< 11; j++)
        {
            ...
            {
                *score += 3;
                if(action3 ==1){/*evanesco*/ evanesco(type, mat); suprimmer(ligne, Y, mat);}
                else if(action2==1){ /*bombarda */bombarda(ligne, Y, mat);}
                else
                {
    ...
    int i,p, type,action2 = 0, action3= 0,X;
        ...
        for(i =0; i< 10; i++)
        {
           ...
           do{
            ...
            {
                *score += 3;
                if(action3==1){/*evanesco*/ suprimmer(X, colonne, mat); evanesco(type, mat); }
                else if(action2==1){ /*bombarda */bombarda(X, colonne, mat);}
                else
                {
